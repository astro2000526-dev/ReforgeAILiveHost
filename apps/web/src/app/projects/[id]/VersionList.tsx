...
export function VersionList({ projectId, versions }: { projectId: string; versions: Version[] }) {
  ...
  async function remove(ids: string[]) {
    ...
    if (!confirm(`ลบ ${ids.length} เวอร์ชัน? (ลบถาวร ย้อนกลับไม่ได้)`)) return
          ...
          <div key={v.id} className={`rounded-md border px-3 py-2 text-xs transition-colors ${sel.has(v.id) ? 'border-primary bg-primary/5' : 'bg-muted/30'}`}>
            <div className="flex items-center gap-2">
              ...
              <span className="ml-auto flex items-center gap-2 shrink-0">
                {v.output_video_url && <a href={`${v.output_video_url}?t=${v.id.slice(0, 8)}`} target="_blank" rel="noreferrer" className="text-primary hover:underline">เปิด ▶</a>}
                <button onClick={() => remove([v.id])} disabled={busy} className="text-red-500 hover:text-red-600" title="ลบเวอร์ชันนี้">🗑</button>
              </span>
            </div>
            {tags.length > 0 && (
